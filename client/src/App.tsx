...
type UrlItem = {
  shortid: string;
  redirecturl: string;
  viewHistory: unknown[]; // you can replace `unknown[]` with your actual type if you know it
};
...
export default function UrlShortener() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const data: { id?: string; shortid: string; redirecturl: string; viewHistory: unknown[] } =
      await res.json();

    if (data.id) {
      setId(data.id);
      setUrls((prev) => [...prev, data]); // push new shortened URL
    }
  ...
  return (
    <div className="bg-gradient-to-br from-gray-100 to-indigo-100 min-h-screen flex items-center justify-center">
